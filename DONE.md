...
- [x] (detection-off-main-actor) 2026-09-02 · **[hi · High · detection/perf]** · `a8baf47` `efa389a` `35e5a8e` `c6b06e9` `939a3b6` · `DetectorPipeline.process` is `@concurrent` and every FENDetector type `nonisolated`, the 64 classifications fan out over a task group, a cancelled detection stops in the scan or between phases, the UI stays live and a landing detection beats engine work started during it (the bot no longer kills a detection); three fixtures pin the output before/after (F-big 3.0 → 2.0 s Debug, none of it on main) — ⚠ verification owed: §5 `(sitting-detection-spinner-paints)`
